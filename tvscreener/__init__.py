...
class ScreenerDataFrame(pd.DataFrame):
    def __init__(self, data, columns: dict, *args, **kwargs):
        ...
        # Reorder columns
        first_columns = ['symbol', 'name', 'description','EMA10','EMA20']
...
class Screener:
    ...
    def get(self, time_interval=TimeInterval.ONE_DAY, print_request=False):
        ...
        with open('assets/json/tColum.json', 'w', encoding='utf-8') as file:
            json.dump(columns, file, indent="\t")
        ...
        if is_status_code_ok(response):
            ...
            #f = open('/Users/ju/Documents/python/tradinviewFile.txt', 'w')
            
           # f.write(data)
            # 기존 json 파일 덮어쓰기
            with open('./tFile.json', 'w', encoding='utf-8') as file:
